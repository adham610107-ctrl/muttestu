...
// ===== THEME & COMFORT EYE — v12 BUG FIX =====
// FIX: dark mode must turn comfort-eye OFF (no sepia on dark bg)
// FIX: applyComfortEye adds CSS class only (no body filter) — z-index/click safe
function toggleTheme() {
    const slider = document.getElementById('theme-slider');
    if (slider && slider.checked) {
        document.body.classList.replace('light-mode', 'dark-mode');
        localStorage.setItem('theme', 'dark');
        // Dark mode: comfort-eye stays as class (CSS handles it safely with pseudo-element)
        // No need to force off — CSS dark-mode.comfort-eye has safe styles (no filter)
    } else {
...
// BUG FIX: comfort-eye is applied as CSS class ONLY.
// CSS uses ::before pseudo-element with pointer-events:none — zero z-index/click interference.
// Body itself has NO filter property — stacking contexts stay intact.
